...
import cn.icodening.console.server.repository.RouterFilterConfigRepository;
 ...
 */
@Service
public class RouterFilterConfigServiceImpl extends AbstractServiceImpl<RouterFilterConfigEntity, RouterFilterConfigRepository>
        implements RouterFilterConfigService, InstanceConfigurationService<RouterFilterConfigEntity> {

    @Override
    public void addRecord(RouterFilterConfigEntity record) {
    ...
    @Override
    public void updateRecord(RouterFilterConfigEntity record) {
        Long id = record.getId();
        if (id == null) {
            throw new AppConsoleException("ID 不能为空");
        }
        RouterFilterConfigEntity recordFromDb = baseRepository.findById(id).orElseThrow(() -> new AppConsoleException("该记录不存在"));
        BeanUtils.copyProperties(record, recordFromDb, BeanPropertyUtil.getNullFieldNames(record));
        save(recordFromDb);
    }

    @Override
    public List<RouterFilterConfigEntity> findInstanceConfiguration(InstanceEntity instanceEntity) {
        ...
        return findBySpecification((Specification<RouterFilterConfigEntity>) (root, query, criteriaBuilder) -> {
            //FIXME 重复代码
    ...
    public String configType() {
        return RouterFilterConfigEntity.class.getName();
    }
}
