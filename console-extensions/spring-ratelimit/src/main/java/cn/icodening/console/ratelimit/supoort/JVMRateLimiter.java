...
public class JVMRateLimiter implements RateLimiter {
    ...
    private Map<String, RateLimitCache> rateLimitCacheMap = new ConcurrentHashMap<>();

    @Override
    public synchronized void refresh(PushData pushData) {
        System.out.println(JVMRateLimiter.class.getName() + ": refresh");
//        Map<String, Object> data = pushData.getData();
...
//        System.out.println(JVMRateLimiter.class.getName() + " map is :" + rateLimitCacheMap);
    ...
    public boolean isAllow(HttpServletRequest request) {
        ...
        System.out.println(JVMRateLimiter.class.getName() + ": best path is " + bestPath);
